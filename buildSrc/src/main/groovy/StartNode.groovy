...
class StartNode  extends DefaultTask {
    ...
    StartNode() {
        ...
        dataDir = getProject().getObjects().directoryProperty()
        serverOptions = getProject().getObjects().property(String.class)
    }
    ...
    void start() {
        project.ant.exec(executable: "${homeDir.get()}/bin/teamcity-server.sh") {
            ...
            env key: 'TEAMCITY_DATA_PATH', path: dataDir.get()
            env key: 'TEAMCITY_LOGS_PATH', path: nodeDir.get().dir('logs')
            env key: 'TEAMCITY_SERVER_OPTS', value: serverOptions.get()
            arg value: 'start'
        }
    }
}
